...
1. 原文同时定义了参数完全相同、仅返回类型不同的两个 `Cross`，C++ 不允许仅按返回类型重载。整理稿拆分为 `cross` 和 `orientation`。
2. 原文若干函数连续写了两个 `return`，后一个永远不可达。整理稿用不同函数或枚举明确结果。
3. 原文 `Rotate` 的写法不直观，整理稿使用标准旋转矩阵。
4. 原文角度计算没有限制 `acos` 参数，可能因浮点误差得到 `NaN`。
5. 原文整点线段函数对水平线段和竖直线段直接返回 `1`，这是错误的。统一公式应为 `gcd(dx,dy)+1`。
6. 格点算法不应使用 `double Point`，否则 `abs`、`gcd` 和面积转换可能产生错误。
7. 原文最小圆覆盖没有处理空点集，且三点共线时会在外接圆计算中除以零。
8. 原文随机化使用多次 `rand()%n` 交换，整理稿改用 `std::shuffle`。
9. 原文自适应 Simpson 使用 `asr(l,r,0)`，会导致错误判定或无意义递归。
10. Simpson 的停止条件应与当前区间容差相关，并使用 `/15` 误差修正。
11. 原文圆与多边形面积交会原地平移输入多边形，造成调用后的数据被修改；整理稿不修改输入。
12. 浮点近似相等不适合直接用于 `std::sort` 比较器，否则可能破坏严格弱序。
13. 圆与圆重合时有无穷多个交点，不能用普通交点数组表达，应先判断位置关系。
14. 外接圆、内切圆、直线交点等接口均存在退化输入，整理稿通过 `bool` 或空数组报告失败。
15. 原文余弦定理第二、第三式的角标存在笔误，整理稿分别修正为 `cos B`、`cos C`。
16. 正多边形中心并不是任意多边形的重心；“外接圆圆心与重心重合”只适用于正多边形。
